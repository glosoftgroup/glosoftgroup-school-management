...
class Exams_m extends MY_Model
{
        ...
        {
                $results = $this->db->where('is_optional', 0)->get('subjects')->result();
                $rr = array();
        ...
        function fetch_marks_list($list_id)
        {
                ...
                foreach ($list as $l)
                {
                        ...
                        if (count($units))
                        {
                                $fn[] = array('subject' => $l->subject, 'marks' => $l->marks, 'units' => $units);
                        }
                        else
                        {
                                $fn[] = array('subject' => $l->subject, 'marks' => $l->marks);
                        }
                }
                return $fn;
        }
        ...
        function fetch_done_list($subject, $list_id)
        {
                ...
                foreach ($list as $l)
                {
                        ...
                        if (count($units))
                        {
                                return array('subject' => $subject, 'mk' => $l->marks, 'units' => $units);
                        }
                        else
                        {
                                return array('subject' => $subject, 'mk' => $l->marks);
                        }
                }
        }
        ...
        function get_subject_result($exam, $student, $subject)
        {
                ...
                if ($srow)
                {
                        ...
                        if (empty($rw))
                        {
                                return array('marks' => 0, 'opt' => $sbk->is_optional);
                        }
                        ...
                        if (count($subs))
                        {
                                ...
                                $sheet = array('subject' => $sbk->name, 'marks' => $rw->marks, 'opt' => $ttp, 'units' => $fnsubs);
                        }
                        else
                        {
                                $sheet = array('subject' => $sbk->name, 'marks' => $rw->marks, 'opt' => $ttp);
                        }
         ...
         * @param int $student
         */
        function get_report($exam, $student)
        {
                $fn = array();
                $rows = $this->db->select('a.id as id,a.remarks as remarks, a.total as total')
                             ->where('exam_type', $exam)
                ...
                if ($srow)
                {
                        ...
                        foreach ($marks as $m)
                        {
                                ...
                                if (count($subs))
                                {
                                        $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'marks' => $m->marks, 'opt' => $ttp, 'units' => $subs);
                                }
                                else
                                {
                                        $sheet[] = array('id' => $m->id, 'subject' => $m->subject, 'marks' => $m->marks, 'opt' => $ttp);
                                }
                        }
                        /**
