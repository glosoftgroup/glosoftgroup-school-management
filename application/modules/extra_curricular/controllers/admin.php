...
parent::__construct();
	...
	public function index()
	{
	   $config = $this->set_paginate_options(); 	//Initialize the pagination class
		...
		$page = ($this->uri->segment(4)) ? $this->uri->segment(4) : 1;
 		...
 		$data['extra_curricular'] = $this->extra_curricular_m->paginate_all($config['per_page'], $page);
	 ...
	 $data['activity']=$this->extra_curricular_m->populate('activities','id','name');
            ...
            //load view
			 if ($this->ion_auth->is_in_group($this->user->id, 3))
              {
                   $this->template->title('Extra Curricular')
				   	->set_layout('teachers')
        ...
        $this->form_validation->set_rules($this->validation());
		 ...
		 //load the view and the layout
		 	 if ($this->ion_auth->is_in_group($this->user->id, 3))
              {
                   $this->template->title('Add Extra Curricular')
				   	->set_layout('teachers')
				->set_partial('teachers_sidebar', 'partials/teachers_sidebar.php')
				->set_partial('teachers_top', 'partials/teachers_top.php')
              ...
              {
		 $this->template->title('Add Extra Curricular ' )->build('admin/create', $data);
			  }
		}		
	}
	...
	{ 
         ...
         if(!$this->extra_curricular_m-> exists($id) )
             {
             $this->session->set_flashdata('message', array( 'type' => 'warning', 'text' => lang('web_object_not_exist') ) );
            redirect('admin/extra_curricular');
              }
        //search the item to show in edit form
        $get =  $this->extra_curricular_m->find($id); 
            ...
            if ($this->form_validation->run() )  //validation has been passed
             {
				...
				//find the item to update
					...
					$done = $this->extra_curricular_m->update_attributes($id, $form_data);
				}
        ...
        if ( $done) 
			...
			else
			{
			 ...
			 if ($this->ion_auth->is_in_group($this->user->id, 3))
              {
                   $this->template->title('Edit Extra Curricular')
				   	->set_layout('teachers')
				->set_partial('teachers_sidebar', 'partials/teachers_sidebar.php')
				->set_partial('teachers_top', 'partials/teachers_top.php')
				   ->build('admin/create', $data);
              ...
              {
             $this->template->title('Edit Extra Curricular ' )->build('admin/create', $data);
			  }
	}
