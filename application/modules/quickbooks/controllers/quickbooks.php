...
class quickbooks extends Public_Controller
{
        ...
        public function __construct()
        {
                ...
                $this->dsn = 'mysql://' . $this->db->username . ':' . $this->db->password . '@' . $this->db->hostname . '/' . $this->db->database;
                //set constants
                $this->set_constants();
        ...
        public function qbwc()
        {
                ...
                $map = array(
                    QUICKBOOKS_IMPORT_INVOICE => array(array($this, '_fetch_invoices_req'), array($this, '_fetch_invoices_resp')),
                    QUICKBOOKS_IMPORT_CUSTOMER => array(array($this, '_fetch_customer_req'), array($this, '_fetch_customer_resp')),
                    QUICKBOOKS_IMPORT_RECEIVEPAYMENT => array(array($this, '_fetch_payments_req'), array($this, '_fetch_payments_resp')),
                );
                // Catch all errors that QuickBooks throws with this function 
                $errmap = array(
                    3070 => array($this, '_error_too_long'),
                    500 => array($this, '_error_500'),
                    1 => array($this, '_error_500'),
                    '*' => array($this, '_error_catchall'));

                // An array of callback hooks
                $hooks = array(
                    QuickBooks_WebConnector_Handlers::HOOK_LOGINSUCCESS => array(array($this, '_hook_login_success')) // call on successful login
                );
                ...
                $handler_options = array(
                    'authenticate' => array($this, 'custom_auth'),
        ...
        {
                QuickBooks_Utilities::log(QB_QUICKBOOKS_DSN, 'Incoming Login Request # ' . $username . ': ' . $password . print_r($params, true));
        ...
        function _hook_login_success($requestID, $user, $hook, &$err, $hook_data, $callback_config)
        {
                ...
                $date = date('Y-m-d H:i:s'); //'2015-04-20 17:01:01';
                ...
                // Queue up Requests
                QuickBooks_Utilities::log(QB_QUICKBOOKS_DSN, 'Queue up Requests ... ');
                //$Queue->enqueue(QUICKBOOKS_IMPORT_INVOICE, 2, QB_PRIORITY_INVOICE);
                $Queue->enqueue(QUICKBOOKS_IMPORT_RECEIVEPAYMENT, 1, QB_PRIORITY_RECEIPT);
                //$Queue->enqueue(QUICKBOOKS_IMPORT_CUSTOMER, 3, QB_PRIORITY_CUSTOMER);
        }
        ...
        function _fetch_invoices_resp($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
        {
                ...
                if ($Doc = $Parser->parse($errnum, $errmsg))
                {
                        ...
                        // QuickBooks_Utilities::log(QB_QUICKBOOKS_DSN, 'XML Request #: ' . print_r($xml, true));
		...
		<QBXML>
			...
			<CustomerAddRq requestID="' . $requestID . '">
                                                        <CustomerAdd>
                                		<Name>Signal Industries Ltd (' . mt_rand() . ')</Name>
				<CompanyName>Signal Industries Ltd</CompanyName>
				<FirstName>Busy</FirstName>
				<LastName>Signal</LastName>
				<BillAddress>
				<Addr1>Landmark Plaza</Addr1>
                                                	<Addr2>1544510 Hurlingham</Addr2>
				<City>4Hurlingham</City>
				<State>Nairobi</State>
				<PostalCode>06268</PostalCode>
				<Country>Kenya</Country>
				</BillAddress>
				<Phone>0722-634-1607</Phone>
				<AltPhone>43-429-0021</AltPhone>
				<Fax>220-429-5183</Fax>
				<Email>bs@gmail.com</Email>
				<Contact>Busy Signal</Contact>
                                                        </CustomerAdd>
			</CustomerAddRq>
                                        </QBXMLMsgsRq>
		</QBXML>';
        ...
        function _fetch_customer_req($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale)
        {
                ...
                $last = '2016-01-01T00:00:00'; //overide last
                //$to = '2015-09-01T07:03:19'; //overide to
                $to = date('Y-m-d\TH:i:s');
                // Build the request
                $xml = '<?xml version="1.0" encoding="utf-8"?>
                  <?qbxml version="12.0"?>
                ...
                if ($Doc = $Parser->parse($errnum, $errmsg))
                {
                        ...
                        foreach ($List->children() as $Customer)
                        {
                                ...
                                if ($ex)
                                {
                                        ...
                                        if ($row->seen == 0)
                                        {
                                                $upd = array('acc_no' => $acc, 'balance' => $bal, 'modified_on' => time());
                                        }
                                        else
                                        {
                                                $upd = array('balance' => $bal, 'modified_on' => time());
                                        }
        ...
        {
                mail('ben@your-domain.com', 'QuickBooks error occured!', 'QuickBooks thinks that ' . $action . ': ' . $ID . ' has a value which will not fit in a QuickBooks field...');
        }
        ...
        {
                // $Queue = QuickBooks_WebConnector_Queue_Singleton::getInstance();
                QuickBooks_Utilities::log(QB_QUICKBOOKS_DSN, 'Error Action # ' . $action . ': ' . print_r($xml, true));
