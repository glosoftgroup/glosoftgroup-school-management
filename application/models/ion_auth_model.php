...
//Get student full details including class and stream	
        public function students_full_details()
        {
                ...
                foreach ($results as $res)
                {
                        ...
                        $arr[$res->id] = $res->first_name . ' ' . $res->last_name . ' (' . $ccc . ' ' . $stt . ')';
                }
        ...
        {
                return $this->db->where('status', 'live')->count_all_results('meetings');
        }
...
//borrowed books
        ...
        function remarks($gs, $marks)
        {
                ...
                if ($marks == 0)
                {
                        return $this->db->where('minimum_marks =', 0)->get('grading_records')->row();
                }
                else
                {
                        $found = $this->db->where('maximum_marks >=', $marks)->order_by('maximum_marks', 'ASC')->get('grading_records')->row();
                        ...
                        {
                                return $this->db->where('minimum_marks <', $marks)->order_by('minimum_marks', 'DESC')->get('grading_records')->row();
                        }
                }
        }
