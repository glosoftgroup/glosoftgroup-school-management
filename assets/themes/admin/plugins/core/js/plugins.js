$(document).ready(function ()
{
    ...
    // Uniform
    $("input:checkbox, input:radio").not('input.ibtn').not('.switchx').uniform();
    // Select2
    if ($(".select").length > 0)
    {
    ...
    if ($(".fTable").length > 0)
        $(".fTable").dataTable({bSort: true,
            "iDisplayLength": flist, "aLengthMenu": [10, 20, 30, 50, 100], // can be removed for basic 10 items per page
            "aoColumnDefs": [{"bSortable": false,
                    "aTargets": [-1, 0]}]});
    if ($(".fpTable").length > 0)
